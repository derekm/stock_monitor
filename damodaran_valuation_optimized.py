...
hist = pd.read_parquet(DATA_DIR / "historical_13f_holdings.parquet")
print(f"Loaded {len(hist)} historical holdings")

# Load CUSIP to ticker map
with open(DATA_DIR / "cusip_ticker_map.json") as f:
...
# PIT mcap panel (not daily_prices.market_cap)
mcap = pd.read_parquet(DATA_DIR / "daily_mcap.parquet", columns=["date", "ticker", "shares", "market_cap"])
mcap["ticker"] = mcap["ticker"].astype(str).str.upper()
mcap["date"] = pd.to_datetime(mcap["date"]).dt.normalize()
print(f"Loaded daily_mcap {len(mcap)} rows")
...
# ============================================================
print("\nComputing actual ownership percentages from daily_mcap...")
...
def merge_asof(df_left, df_right, left_on, right_on, by=None):
    left = df_left.copy()
    right = df_right.copy()
    left[left_on] = pd.to_datetime(left[left_on]).dt.normalize()
    right[right_on] = pd.to_datetime(right[right_on]).dt.normalize()
...
panel = mcap.rename(columns={"ticker": "held_ticker", "date": "as_of_date",
                             "shares": "held_shares_outstanding", "market_cap": "held_market_cap"})
agg_mc = merge_asof(
...
print(f"  SO {agg_mc['held_shares_outstanding'].notna().sum()} / {len(agg_mc)}  "
      f"MC {agg_mc['held_market_cap'].notna().sum()} / {len(agg_mc)}")
...
agg_mc.to_parquet(DATA_DIR / "actual_ownership_percentages.parquet", index=False)
...
lt_fund = pd.read_parquet(DATA_DIR / "quarterly_lookthrough_fundamentals_extended.parquet")
...
for q, q_ts in tqdm(zip(quarters, quarter_timestamps), total=len(quarters), desc="Valuing filers"):
    ...
    # Parent mcap from PIT panel (asof quarter-end)
    q_mcap = mcap[mcap["date"] <= q_ts].drop_duplicates("ticker", keep="last").set_index("ticker")
    parent_mcs = {}
    for filer in operating_filers:
        if filer in q_mcap.index:
            mc = q_mcap.loc[filer, "market_cap"]
            if pd.notna(mc) and mc > 0:
                parent_mcs[filer] = float(mc)
...
val_df.to_parquet(DATA_DIR / "damodaran_crossholdings_valuation.parquet", index=False)
