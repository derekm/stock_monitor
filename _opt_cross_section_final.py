...
def _load_fundamentals_simple(fund: pd.DataFrame, rebal_dates: list[pd.Timestamp], sector_map: dict) -> dict:
    """Load all fundamentals and return dict: rebal_date -> DataFrame(ticker -> value_z, quality_z, sector)"""
    fund_reset = fund.reset_index()
    fund_reset["as_of_date"] = pd.to_datetime(fund_reset["as_of_date"])
    fund_reset = fund_reset.sort_values(["ticker", "as_of_date"])
    ...
    for rb in rebal_dates:
        ...
        if not mask.any():
            ...
            result = result[["value_z", "quality_z", "sector"]]
        result.attrs["rebal_date"] = rb
        # return as dict: ticker -> (value_z, quality_z, sector)
        result = result[["value_z", "quality_z", "sector"]]
        result = result.reset_index()
        result = result.set_index("ticker")
        result = result[["value_z", "quality_z", "sector"]]
...
# OK, let me just write the full optimized file properly