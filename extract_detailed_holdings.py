#!/usr/bin/env python3
"""
extract_detailed_holdings.py — Extract detailed holdings from public companies' 10-K/10-Q XBRL filings.

This version properly parses inline XBRL with dimensional context to extract:
1. Investment breakdown by instrument type (via FinancialInstrumentAxis)
2. Equity method investments with issuer details
3. Fair value hierarchy breakdowns
4. Other dimensional breakdowns available in the filing

Output: detailed_holdings.parquet with columns:
  filer_ticker, as_of_date, concept, dimension_axis, dimension_member, value, unit
"""
...
# Investment-related concepts to extract
INVESTMENT_CONCEPTS = {
...
def get_recent_filings(submissions: Dict, forms: Tuple[str, ...] = ("10-K", "10-Q"), limit: int = 5) -> List[Dict]:
    """Extract recent 10-K/10-Q filings from submissions"""
...
def download_filing(cik: str, accession_no_dash: str, primary_document: str) -> Optional[str]:
    """Download the primary filing document (usually .htm with inline XBRL)"""
...
def parse_inline_xbrl(html_content: str) -> Tuple[Dict[str, Dict], Dict[str, Dict], List[Dict]]:
    """
    Parse inline XBRL content to extract:
    1. Contexts (with dimensions/segments)
    2. Units
    3. Facts (numeric and non-numeric) with their contextRef and unitRef
    """
    contexts = {}
    units = {}
    facts = []
    ...
    for full_ctx, ctx_id in ctx_matches:
        ...
        if segment_match:
            ...
            for dim, member_content in typed_matches:
                # Extract the value from typed member
    ...
    # Extract numeric facts (ix:nonFraction) - use flexible regex since attribute order varies
    ...
    for attrs, value in frac_matches:
        # Extract name, contextRef, unitRef from attrs
        ...
        # Clean value (remove commas, handle negative in parentheses)
...
def extract_holdings_from_filing(
    ticker: str,
    cik: str,
    filing: Dict,
    html_content: str
) -> List[Dict]:
    """Extract all investment-related holdings from a single filing"""
    contexts, units, facts = parse_inline_xbrl(html_content)
    
    # Determine filing date (prefer report_date, fallback to filing_date)
    ...
    # Filter for investment-related concepts
    investment_facts = []
    for fact in facts:
        ...
        # Check if this is an investment concept we care about
        is_investment = any(
            inv_concept in concept for inv_concept in INVESTMENT_CONCEPTS.values()
        )
        
        # Also check for any concept containing investment-related keywords
        investment_keywords = [
        ...
        # Include all dimensions for completeness
...
def process_ticker(ticker: str, cik_map: Dict[str, str], max_filings: int = 5) -> List[Dict]:
    ...
    for filing in filings:
        ...
        holdings = extract_holdings_from_filing(ticker, cik, filing, html_content)
...
def main():
    parser = argparse.ArgumentParser(description="Extract detailed holdings from SEC XBRL filings")
    ...
    for ticker in tqdm(tickers, desc="Processing tickers"):
        ...
        print(f"  Extracted {len(holdings)} investment facts")
        all_holdings.extend(holdings)
