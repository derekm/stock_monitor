...
Reads snapshots of `daily_prices/` and `fundamentals.parquet` (does not hold the live files). Writes month-end long `expected_returns_decomp.parquet`: `date`, `ticker`, pillar ranks, `expected_return`.
