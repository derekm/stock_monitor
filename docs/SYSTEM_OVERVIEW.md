...
| **Taleb layer** | Fat-tail index, gap risk, fragility screen (micro) + **macro debt fragility (Keen/Minsky)** , ergodicity/ruin, barbell check, **hidden-optionality audit** (decision-flip rates), Forecasting-Paradox uncertainty (Student-t predictive + dials of doubt) |
| **Ops** | Alerts (price + fundamentals), daily automation (33 jobs), DuckDB-Wasm dashboard, analytics API |
