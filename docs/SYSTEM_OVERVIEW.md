...
- **LLM forecast brief is starved of coverage, not of ideas.** `forecast_llm.py` can only cite what is dense, ticker-keyed, and wordable. On disk (2026-08-29) these layers are too thin or too easy to misread to drive a desk note:
  ...
  - **LLM series:** `forecast_llm.py` writes **last HMM date only** (no `--lookback` date loop). Default universe is the coverage-gated set (**365**). Job profiles in one long table (`profile` column): **`value`** (too-expensive means not up; omit growth/12-1/r63 when expensive), **`exuberant`** (crowd can keep paying up; growth/12-1/r63 stay in the brief), **`compounder`** (leftover cash + ROIC−WACC beat can carry an expensive name; 3y growth stays, 12-1/r63 stay off). Innermost loop is profile per ticker; SYSTEM lectures only facts in that ticker's brief. `llm.reset()` before every generation so jobs are not KV order-dependent. Resume key is `(date, ticker, profile)`. Stratified test was **16×1 date** (2026-08-24). Conformal stays blocked until a multi-date series exists. 3B default; `max_tokens=120`; live prompts ~297–385 tokens inside `n_ctx=1024` (headroom ~520). Stay **one-shot**. Do not let the GGUF write the operating brief. Press is a sidecar (`ticker_news_press.parquet`) — one gated sentence, not a reason to own. All post-Docling 3B stages — mentions, press, and the forecast itself — run on Intel Iris Xe Vulkan (`.venv-xpu`, `GGML_VK_VISIBLE_DEVICES=0`); MX550/CUDA measured ~6 tok/s decode vs ~15 on Xe and is retired. Headline-only desk notes (`--notes`) were removed — mentions + press replaced them. The daily DAG serializes the chain firehose+Docling → mentions → press → llm_forecast (forecast consumes the freshest press lines, so the stages never overlap). Python owns the dossier. Do not spend headroom on sector/macro tape or unitless scores.
