...
# CLEAN: dedupe by ticker+date, keep recent stationary tail (~2520 trading days ~10y)
raw = pd.read_parquet(b.PRICES)
raw = raw.drop_duplicates(subset=['ticker', 'date']).sort_values(['ticker', 'date'])
RECENT = 2520  # last 10y of trading days -> stationary, comparable scale
raw = raw.groupby('ticker', group_keys=False).tail(RECENT)
print(f"cleaned rows: {len(raw)} (dedup+recent), tickers: {raw['ticker'].nunique()}", flush=True)
...
def make_windows(s, cap):
    # full-ticker raw rolling, stride=1, capped at `cap` by even downsample
