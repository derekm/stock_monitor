import time, numpy as np, torch, threading, subprocess
...
t0 = time.time(); train_one('AEP'); train_one('BA')
serial = time.time() - t0; th.join()
print(f'SERIAL 2 tickers: {serial:.1f}s  gpu_mean={sum(out)/len(out):.0f}%', flush=True)
...
print(f'PARALLEL 2 tickers: {par:.1f}s  gpu_mean={sum(out)/len(out):.0f}%  speedup={serial/par:.2f}x', flush=True)
